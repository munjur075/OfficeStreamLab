...
class PaypalCheckOutView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.is_subscribe:
            return Response(
                {"message": "You are already subscribed! Cancel your current subscription first."},
        ...
        paypal_checkout = {
            ...
            "return": request.build_absolute_uri(reverse("subscription:paypal-checkout-success")),
            "cancel_return": request.build_absolute_uri(reverse("subscription:paypal-checkout-failed")),
            "custom": str(user.id),
        }

        # PayPal Sandbox or Live URL
        base_url = "https://www.sandbox.paypal.com/cgi-bin/webscr" if settings.PAYPAL_TEST else "https://www.paypal.com/cgi-bin/webscr"
        checkout_url = (
            f"{base_url}?cmd=_xclick"
            f"&business={settings.PAYPAL_RECEIVER_EMAIL}"
            f"&amount={plan_obj.price}&currency_code=USD"
            f"&item_name={plan_obj.name}&invoice={invoice_id}"
            f"&notify_url={paypal_checkout['notify_url']}"
            f"&return={paypal_checkout['return']}"
            f"&cancel_return={paypal_checkout['cancel_return']}"
            f"&custom={user.id}"
        )
