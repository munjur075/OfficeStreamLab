...
# -------------------- Checkout Session --------------------
class CreateStripeRentedCheckoutSessionView(APIView):
        ...
        try:
            checkout_session = stripe.checkout.Session.create(
                ...
                success_url=request.build_absolute_uri(
                    reverse("movie:stripe_rented_checkout_success")
                ) + "?session_id={CHECKOUT_SESSION_ID}",
                cancel_url=request.build_absolute_uri(
                    reverse("movie:stripe_rented_checkout_cancel")
                ),
            )
...
# -------------------- Success / Cancel --------------------
def stripe_rented_checkout_success_view(request):
    return JsonResponse({
        "status": "success",
        "message": "Film rentedd successfully."
    })


def stripe_rented_checkout_cancel_view(request):
    return JsonResponse({
        "status": "cancelled",
        "message": "Film rented cancelled."
    })
...
#@M.Alom
# -------------------- Stripe Webhook --------------------
class StripeWebhookRentedView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if event["type"] == "checkout.session.completed":
            ...
            rent_hour = session["metadata"].get("rent_hour", "0")
        ...
        # ---------------- FAILED PAYMENT ----------------
            ...
            try:
                ...
                Transaction.objects.create(
                    ...
                    status="failed",
                    txn_id=payment_id,
                    description="Failed film rented"
                )
