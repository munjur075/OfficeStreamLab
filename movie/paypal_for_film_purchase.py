...
# ---------------- CREATE FILM PURCHASE ----------------
class CreatePaypalFilmPurchaseView(APIView):
    ...
    def post(self, request):
        ...
        payment = paypalrestsdk.Payment({
            ...
            "redirect_urls": {
                "return_url": request.build_absolute_uri(
                    reverse("movie:paypal_purchase_execute")
                ) + f"?film_id={film.id}&txn_id={txn_id}&ref={referral_code or ''}",
                "cancel_url": request.build_absolute_uri(
                    reverse("movie:paypal_purchase_cancel")
                ),
            },
            "transactions": [{
                "item_list": {"items": [{
                    "name": f"Purchase Film: {film.title}",
                    "sku": str(uuid.uuid4()),
                    "price": f"{price:.2f}",
                    "currency": "USD",
                    "quantity": 1,
                ...
                "description": f"Purchase {film.title}",
            }],
        })
        ...
        if payment.create():
            ...
            Transaction.objects.create(
                ...
                txn_id=txn_id,
                paypal_token=token,
                description=f"Pending PayPal purchase for {film.title}"
            )
            return Response({"approvalUrl": approval_url})

        return Response({"message": payment.error}, status=500)

# ---------------- EXECUTE FILM PURCHASE ----------------
class ExecutePaypalFilmPurchaseView(APIView):
    authentication_classes = []  # PayPal redirect (no JWT)
    permission_classes = []
        ...
        if payment.execute({"payer_id": payer_id}):
            try:
                with transaction.atomic():
                    ...
                    # ---- Revenue Split (based on net_amount) ----
...
# ---------------- CANCEL FILM PURCHASE ----------------
def paypal_film_cancel_view(request):
    token = request.GET.get("token")
    if token:
        Transaction.objects.filter(paypal_token=token, status="pending").update(status="failed")
    return Response({"status": "cancelled", "message": "Film purchase cancelled"})
